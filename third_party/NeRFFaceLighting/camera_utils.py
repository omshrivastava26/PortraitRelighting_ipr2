...
class GaussianCameraPoseSampler:
    ...
    def sample(
        ...
        return create_cam2world_matrix(forward_vectors, camera_origins, z)
...
def create_cam2world_matrix(forward_vector, origin, z=0):
    """
    Takes in the direction the camera is pointing and the camera origin and returns a cam2world matrix.
    Works on batches of forward_vectors, origins. Assumes y-axis is up and that there is no camera roll.
    """
    # NOW support camera roll
    ...
    z = torch.ones([1, 1], device=origin.device) * z
    ones = torch.ones([1, 1], device=origin.device)
    zeros = torch.zeros([1, 1], device=origin.device)
    rot_z = (
        torch.cat(
            [
                torch.cos(z),
    ...
    cam2world = (rot_z @ translation_matrix @ rotation_matrix)[:, :, :]
