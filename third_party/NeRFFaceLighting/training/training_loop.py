...
def setup_snapshot_image_grid(training_set, random_seed=0):
        ...
        for idx in range(len(training_set)):
            label = tuple(
                sum(
                    [
                        list(r.flat[::-1])
                        for r in training_set.get_details(idx).raw_label
                    ],
                    [],
                )
            )
    ...
    # Load data.
    images, labels, lights = zip(*[training_set[i] for i in grid_indices])
    return (gw, gh), np.stack(images), [np.stack(labels), np.stack(lights)]
...
def training_loop(
    ...
    torch.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = (
        False  # Improves numerical accuracy.
    )
    ...
    G = (
        dnnlib.util.construct_class_by_name(
            c_dim=training_set.label_dim[0],
            l_dim=training_set.label_dim[1],
            **G_kwargs,
            **common_kwargs,
        )
        .train()
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for res in D.block_resolutions:
            getattr(D, f"l{res}").load_state_dict(
                getattr(resume_data["D"], f"b{res}").state_dict()
            )
    ...
    if rank == 0:
        ...
        img = misc.print_module_summary(G, [z, c, l])
        misc.print_module_summary(D, [img, c, l])
    ...
    loss = dnnlib.util.construct_class_by_name(
        ...
        sh_std=training_set._get_raw_labels()[1].std(axis=0)[None, ...],
        **loss_kwargs,
    ...
    if rank == 0:
        ...
        grid_z = torch.randn([labels[0].shape[0], G.z_dim], device=device).split(
            batch_gpu
        )
        grid_c = torch.from_numpy(labels[0]).to(device).split(batch_gpu)
        grid_l = torch.from_numpy(labels[1]).to(device).split(batch_gpu)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function("data_fetch"):
            phase_real_img, phase_real_c, phase_real_l = next(training_set_iterator)
            phase_real_img = (
                phase_real_img.to(device).to(torch.float32) / 127.5 - 1
            ).split(batch_gpu)
            phase_real_c = phase_real_c.to(device).split(batch_gpu)
            phase_real_l = phase_real_l.to(device).split(batch_gpu)
            ...
            all_gen_c = [training_set.get_label(_)[0] for _ in _condition_index]
            ...
            all_gen_l = [training_set.get_label(_)[1] for _ in _condition_index]
            all_gen_l = torch.from_numpy(np.stack(all_gen_l)).pin_memory().to(device)
            all_gen_l = [
                phase_gen_l.split(batch_gpu)
                for phase_gen_l in all_gen_l.split(batch_size)
            ]

        # Execute training phases.
        for phase, phase_gen_z, phase_gen_c, phase_gen_l in zip(
            phases, all_gen_z, all_gen_c, all_gen_l
        ):
            ...
            for real_img, real_c, real_l, gen_z, gen_c, gen_l in zip(
                phase_real_img,
                phase_real_c,
                phase_real_l,
                phase_gen_z,
            ...
            with torch.autograd.profiler.record_function(phase.name + "_opt"):
                params = [
                    ...
                    if param.numel() > 0 and param.grad is not None
                ]
                if len(params) > 0:
        ...
        if (
            ...
            with torch.no_grad():
                out = [
                    G_ema(z=z, c=c, l=l, noise_mode="const")
                    for z, c, l in zip(grid_z, grid_c, grid_l)
                ]
            ...
            # images_pre_shading = torch.cat([o['image_pre_shading'].cpu() for o in out]).numpy()
            images_shading = torch.cat([o["image_shading"].cpu() for o in out]).numpy()
            # images_normal = torch.cat([o['image_normal'].cpu() for o in out]).numpy()
            save_image_grid(
                images,
            ...
            save_image_grid(
                images_albedo,
                os.path.join(run_dir, f"fakes{cur_nimg//1000:06d}_albedo.png"),
                drange=[-1, 1],
                grid_size=grid_size,
            )
            # save_image_grid(images_pre_shading, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_pre_shading.png'), drange=[0, 1], grid_size=grid_size)
            save_image_grid(
                images_shading,
                os.path.join(run_dir, f"fakes{cur_nimg//1000:06d}_shading.png"),
                drange=[images_shading.min(), images_shading.max()],
                grid_size=grid_size,
            )
            # save_image_grid(images_normal, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_normal.png'), drange=[-1, 1], grid_size=grid_size)
                ...
                if module is not None:
                    if num_gpus > 1:
                        misc.check_ddp_consistency(
                            module, ignore_regex=r".*\.[^.]+_(avg|ema)"
                        )
        ...
        if (snapshot_data is not None) and (len(metrics) > 0):
            ...
            # Disabling computation of metrics for purpose of speeding up ...
