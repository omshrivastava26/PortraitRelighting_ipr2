...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self._xflip[idx]:
            ...
            image = image[:, :, ::-1]
        label, light = self.get_label(idx)
        return image.copy(), label, light

    def get_label(self, idx):
        labels = [
            _raw_labels[self._raw_idx[idx]] for _raw_labels in self._get_raw_labels()
        ]
        out = []
        for label in labels:
    ...
    def get_details(self, idx):
        ...
        d.raw_label = [
            _raw_labels[d.raw_idx].copy() for _raw_labels in self._get_raw_labels()
        ]
        return d
...
class ImageFolderDataset(Dataset):
    ...
    def _load_raw_image(self, raw_idx):
        ...
        with self._open_file(fname) as f:
            if pyspng is not None and self._file_ext(fname) == ".png":
                ...
                image = np.array(
                    PIL.Image.open(f)
                )  # .resize((self._resolution, self._resolution), PIL.Image.ANTIALIAS)
        if image.ndim == 2:
    ...
    def _load_raw_labels(self):
        ...
        shs = dict(shs)
        shs = [shs[fname.replace("\\", "/")] for fname in self._image_fnames]
        shs = np.array(shs)
