...
@persistence.persistent_class
class SynthesisLayer_style(torch.nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels.
        out_channels,  # Number of output channels.
...
@persistence.persistent_class
class ToRGBLayer_style(torch.nn.Module):  #! STYLED
    def __init__(
        self,
        in_channels,
        out_channels,
    ...
    def forward(self, x, w, fused_modconv=True):
        # styles = self.affine(w) * self.weight_gain
        styles = w[:, : self.in_channels] * self.weight_gain
        x = modulated_conv2d(
            x=x,
            weight=self.weight,
            styles=styles,
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(
        ...
        **layer_kwargs,
        ...
        inplace_add = True
        if img is not None:
            if img.shape[-1] == self.resolution // 2:
                misc.assert_shape(
                    img,
                    [
                        None,
...
@persistence.persistent_class
class SynthesisBlock_style(torch.nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels, 0 = first block.
        out_channels,  # Number of output channels.
        ...
        **layer_kwargs,  # Arguments for SynthesisLayer.
        ...
        if in_channels != 0:
            self.conv0 = SynthesisLayer_style(
                in_channels,
                out_channels,
                w_dim=w_dim,
                resolution=resolution,
        ...
        self.conv1 = SynthesisLayer_style(
            out_channels,
            out_channels,
            w_dim=w_dim,
            resolution=resolution,
