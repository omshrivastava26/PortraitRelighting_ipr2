...
sys.path.append("..")
sys.path.append("NeRFFaceLighting")

from third_party.NeRFFaceLighting.torch_utils import misc
from third_party.NeRFFaceLighting.torch_utils.ops import upfirdn2d
from third_party.NeRFFaceLighting.training.dual_discriminator import filtered_resizing
...
class DPR(torch.nn.Module):
    def __init__(self, device: torch.device):
        ...
        network.load_state_dict(
            torch.load(
                os.path.join(
                    os.path.dirname(__file__), "trained_model", "trained_model_03.t7"
                )
            )
        )
    ...
    def extract_lighting(self, img: torch.Tensor):
        ...
        img = rgb_to_lab(img.clamp(-1, 1) / 2.0 + 0.5)
        L = img[:, :1, :, :] / 100.0
    ...
    def exert_lighting(self, img: torch.Tensor, sh: torch.Tensor):
        ...
        img = torch.cat((L_p * 100.0, img[:, 1:2], img[:, 2:3]), dim=1)
        return filtered_resizing(
            lab_to_rgb(img) * 2 - 1, size=shape, f=self.resample_filter
        )
